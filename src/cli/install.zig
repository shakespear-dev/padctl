...
fn generateReconnectScript(allocator: std.mem.Allocator, prefix: []const u8) ![]const u8 {
    return std.fmt.allocPrint(allocator,
        ...
        \\# Find the first installed mapping in /etc/padctl/mappings/ and apply it.
        \\sleep 2
        \\for f in /etc/padctl/mappings/*.toml; do
        \\    [ -f "$f" ] || continue
        \\    name="$(basename "$f" .toml)"
        \\    {s}/bin/padctl switch "$name" --socket /run/padctl/padctl.sock 2>/dev/null && break
        \\done
        \\
    , .{prefix});
}
        ...
        if (mappings_src) |src| {
            for (opts.mappings) |mapping_name| {
                installMapping(allocator, mapping_name, destdir, src, opts.force_mapping) catch |err| {
                    _ = std.posix.write(std.posix.STDERR_FILENO, "warning: mapping '") catch {};
            ...
            _ = std.posix.write(std.posix.STDERR_FILENO, "warning: mappings directory not found near executable or current working directory\n") catch {};
        }
    }
