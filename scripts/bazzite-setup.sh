...
install_boot_reprobe() {
    ...
    local unit="/etc/systemd/system/padctl-boot-reprobe.service"
    info "Installing boot-reprobe service (clears the stray 'Generic' pad at boot)..."

    sudo tee "$helper" >/dev/null <<'HELPER'
#!/bin/bash
# padctl-boot-reprobe — Auto-generated by bazzite-setup.sh; do not edit.
# Wait for the padctl daemon IPC socket, then re-fire udev add events so the
# socket-gated 61-padctl-driver-block rule unbinds the kernel driver (xpad)
# from managed pads — fixing the boot race where the kernel grabs the pad
# before padctl is up and a stray generic controller leaks into Steam.
for _ in $(seq 1 60); do
    if ls /run/user/*/padctl.sock /run/padctl/padctl.sock >/dev/null 2>&1; then
        break
    fi
    sleep 1
done
...
[Unit]
Description=padctl boot reprobe (unbind kernel drivers from managed pads once the daemon socket is up)
...
UNIT
    ...
    sudo systemctl daemon-reload >/dev/null 2>&1 || true
    if sudo systemctl enable padctl-boot-reprobe.service >/dev/null 2>&1; then
        ok "boot-reprobe service installed and enabled"
    else
        warn "boot-reprobe service installed but could not be enabled — run: sudo systemctl enable padctl-boot-reprobe.service"
    fi
}
